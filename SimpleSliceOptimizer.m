classdef SimpleSliceOptimizer < SliceOptimizer
  ...
  methods (Access = protected)
    ...
    function As = getAs_res(this, flow_owner, alpha_f)
      ...
      Nvf = this.hs.NumberVNFs;       % For 'single-function', NV=1.
      ...
      for f = 1:Nvf
        if nargin >= 3   % used when treat all VNFs as one function.
          ...
          af = this.Parent.VNFTable{this.hs.VNFList(f),{'ProcessEfficiency'}};
    ...
    function [x, fval] = optimize(this, params, options)
      if options.SlicingMethod.IsSingle
        ...
        [xs, fval, exitflag, output] = ...
          fmincon(@(x)SimpleSlice.fcnSocialWelfare(x,this.hs,options), ...
      ...
      elseif options.SlicingMethod.IsPricing
        [xs, fval, exitflag, output] = ...
          fmincon(@(x)SimpleSlice.fcnProfit(x, this.hs, options), ...
          params.x0, params.As, params.bs, params.Aeq, params.beq, ...
          params.lb, params.ub, [], options.fmincon_opt);
      else
      end
    ...
    function [tf, vars] = postProcessing(this)
      ...
      tol_zero = this.hs.Parent.options.NonzeroTolerance; % TODO: replace with the optimizer.options.
      ...
      if ~isempty(index_violate)
        ...
        post_process = this.hs.Parent.options.PostProcessing; % TODO: replace with the optimizer.options.
        switch post_process
          case 'round'
          ...
          case {'drop','recover'}
            ...
            tol_con = this.hs.Parent.options.ConstraintTolerance; % TODO: replace with the optimizer.options.
