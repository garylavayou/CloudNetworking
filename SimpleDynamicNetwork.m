classdef SimpleDynamicNetwork < DynamicNetwork
	
	methods
		function this = SimpleDynamicNetwork(node_opt, link_opt, VNF_opt, net_opt)
	...
	methods
		function op = getOptimizer(this, options)
			if nargin == 1
				this.op = SlimpleDynamicNetworkOptimizer(this);
			else
				this.op = SlimpleDynamicNetworkOptimizer(this, options);
			end
			op = this.op;
		end
	end
