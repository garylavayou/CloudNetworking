...
%% Dual decomposition
% # An initial point of the dual variable is given as ��=1 (all constraints are active)
...
% for ��>=0. the dual variable after the updates must still be in the feasible domain.
% # Determine maximum step length (less than the feasible step length) of ��_n
% for g(z)>=0.
% # Determine maximum step length of ��_pf for g(z)>=0.
% After finding the maximum step length for ��_n, we calculate the step length for ��_pf
...
% since in some direction the dual variable has reached the boundaries of �� or g(z,��).
%     for i = 1:NS
...
%         a1 = min(min(alpha_npf, [], 3), [], 2);		% find the maximum (max-min) step length for ��_n in slice i.
...
%         gradz = node_unit_cost(nid) + phis_n + lambda.n(nid)-lambda_pf;
%         H_npf = repmat(full(this.slices{i}.I_dc_path), 1, 1, NFi);
%         % if delta_lambda_pf<= 0, any positive step length for ��_pf is feasible.
