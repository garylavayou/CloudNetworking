...
while true
    ...
    while true
        ...
        aggr_node_load = sum(node_load, 1)';
        aggr_link_load = sum(link_load, 1)';
        ide = aggr_link_load>link_capacity;
        idn = aggr_node_load>node_capacity;
        b_feasible = true;
        ...
        elseif ~b_ascent && b_feasible
            ...
            if b_partial_search
                ...
                step_length = step_length/2;
                if step_length/norm([lambda.e; lambda.n]) < 10^-8
            ...
            if b_partial_search
                ...
                if step_length/norm([lambda.e; lambda.n]) < 10^-6
                    ...
                    partial_step.e(ide) = partial_step.e(ide).*...
                        (-aggr_link_load(ide)./link_capacity(ide));        
                    partial_step.n(idn) = partial_step.n(idn).*...
                        (-aggr_node_load(idn)./node_capacity(idn)-1);
                    temp_lambda.e = lambda.e + partial_step.e.*delta_lambda.e;
                    temp_lambda.n = lambda.n + partial_step.n.*delta_lambda.n;
                    num_ascent_reject = 0;
                else
...
dual_fval = -(old_dual_fval - epsilon*(NN-1));
utility = 0;
for i = 1:NS
    utility = utility + this.slices{i}.weight*sum(log(this.slices{i}.FlowTable.Rate));
    node_load(i,this.slices{i}.VirtualNodes.PhysicalNode) = this.slices{i}.VirtualNodes.Load;
    link_load(i,this.slices{i}.VirtualLinks.PhysicalLink) = this.slices{i}.VirtualLinks.Load;
end
aggr_node_load = sum(node_load,1)';
aggr_link_load = sum(link_load,1)';
