...
%% Display option
% iter-final.
function [output, runtime] = optimizeResourcePrice3(this, init_price, options)
if nargin <= 2
...
stop_cond3 = ...
    this.checkProfitRatio(aggr_node_load, aggr_link_load, node_price, link_price, options);
slice_profit = zeros(NS,1);
...
while stop_cond1 && stop_cond2 && stop_cond3
    ...
    if assert_link_1 && assert_node_1
        ...
        partial_link_violate = false(NL, 1);
        partial_node_violate = false(NN, 1);
        stop_cond3 = this.checkProfitRatio(...
            aggr_node_load, aggr_link_load, node_price, link_price, options);
    else
        b_first = false;
        link_price(b_link) = link_price(b_link) + delta_link_price(b_link);
        node_price(b_node) = node_price(b_node) + delta_node_price(b_node);
...
if ~stop_cond3
    ...
    while true
        ...
        for s = 1:NS
            this.slices{s}.VirtualLinks.Price = ...
                temp_link_price(this.slices{s}.VirtualLinks.PhysicalLink);
            this.slices{s}.VirtualNodes.Price = ...
                temp_node_price(this.slices{s}.VirtualNodes.PhysicalNode);
            if nargout == 2
                tic;
            end
            [~, node_load(:,s), link_load(:,s)] ...
        ...
        aggr_node_load = sum(node_load, 2);
        aggr_link_load = sum(link_load, 2);
        if this.checkProfitRatio(...
                aggr_node_load, aggr_link_load, temp_node_price, temp_link_price, options2)
            link_price = temp_link_price;
            node_price = temp_node_price;
            break;
        end
        if this.checkProfitRatio(...
                aggr_node_load, aggr_link_load, temp_node_price, temp_link_price, options)
            h = alpha;
        else
            l = alpha;
        end
