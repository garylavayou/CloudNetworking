...
defaultopts = Dictionary('ResidualCapacity', capacities, ...
	...
	'PricingPolicy', 'quadratic-price', ... % override the default
	'Stage', 'temp', ...
	'bFinal', false, ...
...
% Initialize Price
prices = this.op.initPrice(slices, unitcost, options);
t0 = 10^-1;     % {1|0.1|0.01}
...
sp_profit = this.SolveSCPCC(slices, prices, options);
...
while true
  ...
  % utilization θ>0), since increasing the price of idle resources will not iNdcrease the
  % profit of SP.
  ...
  sp_profit_new = this.SolveSCPCC(slices, prices, options);
  %% Stop condtion
...
if ~b_profit_ratio || ~b_forced_break
  ...
  while true  %|| (h-l) > 0.05
    ...
    for i = 1:2
			sp_profit_new(i) = this.SolveSCPCC(slices, price_middle(i), options);
    end
...
while true
  ...
  number_iter = number_iter + 1;
  this.SolveSCPCC(slices, prices, options);
  k = k+1;
end
